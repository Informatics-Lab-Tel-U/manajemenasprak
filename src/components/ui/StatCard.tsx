    ...
    >
      ...
      <CardHeader className="flex items-start justify-between space-y-0 pb-3 relative z-10">
        <div className="space-y-1 flex-1">
          <p className="text-sm font-medium text-muted-foreground group-hover:text-foreground/70 transition-colors uppercase tracking-wider text-[10px]">
            {title}
          </p>
          {loading ? (
            <Skeleton className="h-9 w-16 mt-1" />
          ) : (
            <h3 className="text-3xl font-bold tracking-tight text-foreground/90 dark:text-foreground">
              {value}
            </h3>
          )}
        </div>
        <div
          className={cn(
            'rounded-lg p-2.5 shrink-0 shadow-sm transition-transform group-hover:scale-110 duration-300',
            style.icon
          )}
        >
