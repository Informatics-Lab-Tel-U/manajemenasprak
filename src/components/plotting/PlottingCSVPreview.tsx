          ...
          <table className="w-full text-sm border-collapse">
            <thead className="bg-muted/50 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-3 py-2.5 text-left border-b text-muted-foreground uppercase text-xs font-semibold">
                  Kode Asprak
                </th>
                <th className="px-3 py-2.5 text-left border-b text-muted-foreground uppercase text-xs font-semibold">
                  Mata Kuliah
                ...
                <th className="px-3 py-2.5 text-left border-b text-muted-foreground uppercase text-xs font-semibold w-[350px]">
                  Resolution (Select Candidates)
                </th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-muted-foreground font-mono text-xs align-top pt-3">
                      {idx + 1}
                    </td>
                    <td className="px-3 py-2 font-mono font-bold align-top pt-3">
                      {row.kode_asprak}
                    </td>
                    ...
                    <td className="px-3 py-2 align-top pt-3">
                      ...
                      {row.status === 'invalid' && (
                        <span className="text-red-500 flex items-center gap-1" title={row.message}>
                          <XCircle size={14} /> {row.message || 'Error'}
                        </span>
                      )}
                      {row.status === 'ambiguous' && (
                        <span className="text-amber-500 flex items-center gap-1">
                          <HelpCircle size={14} /> Ambiguous
                        </span>
                      )}
                    </td>
