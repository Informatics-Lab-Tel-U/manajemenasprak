...
import { FileSpreadsheet, Upload, FileText, X, Download, AlertTriangle, Check, Loader2, ArrowLeft, Save, Copy, CopyX, CheckCircle } from 'lucide-react';
...
import { CreateJadwalInput } from '@/services/jadwalService';
...
export default function JadwalImportCSVModal({
  ...
  const validateConflicts = async (rows: JadwalPreviewRow[]): Promise<JadwalPreviewRow[]> => {
      ...
      const getKey = (hari: string, sesi: number | string, ruangan: string) => `${hari}-${sesi}-${ruangan}`;
      ...
      return rows.map((row, idx) => {
          ...
          const fullKey = getFullKey(newRow.id_mk, newRow.kelas, newRow.hari, newRow.sesi, newRow.ruangan || '');
      ...
      Papa.parse<RawCSVRow>(file, {
        ...
        complete: async (results) => {
          ...
          data.forEach((row, idx) => {
             ...
             const targetMK = candidates.find(mk => 
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={cn("flex max-h-[min(800px,90vh)] flex-col gap-0 p-0", step === 'preview' ? "sm:max-w-5xl" : "sm:max-w-lg")}>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        {term && <span className="text-xs font-normal text-muted-foreground bg-muted px-2 py-0.5 rounded">Term: {term}</span>}
                      </div>
                      {fileName && (
                    ...
                    <div
                      ...
                      className={cn(
                        ...
                        isDragActive ? 'border-primary bg-primary/5' : 'border-border bg-transparent hover:border-primary/50'
                      )}
                    >
                      <input {...getInputProps()} />
                    ...
                    <div className="bg-muted/30 p-4 rounded-lg border border-border/50">
                        ...
                        <div className="flex flex-wrap gap-2 mb-1">
                          {['Kelas', 'Mata Kuliah', 'Hari', 'Sesi', 'Jam', 'Ruangan', 'Total Asprak', 'Dosen'].map((col) => (
                        ...
                        <table className="w-full text-sm border-collapse">
                            <thead className="bg-muted/50 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="px-3 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider border-b border-border">Mata Kuliah</th>
                                    <th className="px-3 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider border-b border-border">Kelas</th>
                                    <th className="px-3 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider border-b border-border">Data Waktu</th>
                                    <th className="px-3 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider border-b border-border">Ruangan</th>
                                    <th className="px-3 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider border-b border-border">Dosen</th>
                                    <th className="px-3 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider border-b border-border">Status</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    return (
                                        <tr key={idx} className={cn(
                                            ...
                                            <td className="px-3 py-2 font-medium">
                                                <div>{row.mkName}</div>
                                                <div className="text-[10px] text-muted-foreground/60 font-mono hidden sm:block">ID: {row.id_mk || '-'}</div>
                                            </td>
                                            <td className="px-3 py-2">{row.kelas}</td>
                                            <td className="px-3 py-2">
                                                <div>{row.hari}, {row.jam}</div>
                                                <div className="text-[10px] text-muted-foreground/60">Sesi {row.sesi}</div>
                                            </td>
                                            <td className="px-3 py-2">{row.ruangan}</td>
                                            <td className="px-3 py-2 truncate max-w-[150px]" title={row.dosen}>{row.dosen}</td>
                                            <td className="px-3 py-2">
                                                {row.status === 'ok' && (
                                                ...
                                                {row.status === 'warning' && (
                                                  <span className="inline-flex items-center gap-1 text-amber-500 text-xs font-medium" title={row.statusMessage}>
                                                    <AlertTriangle size={14} /> {row.statusMessage}
                                                  </span>
                                                )}
                                                {row.status === 'error' && (
                                                  <span className="inline-flex items-center gap-1 text-red-500 text-xs font-medium" title={row.statusMessage}>
                                                    <AlertTriangle size={14} /> {row.statusMessage}
                                                  </span>
                                                )}
                                            </td>
                  ...
                  <div className="flex justify-between items-center pt-2">
                     ...
                     <div className="flex items-center gap-3">
                       ...
                       <Button 
                         ...
                         variant="default"
                       >
                         {saving ? <Loader2 className="animate-spin mr-2" size={16} /> : <Save size={16} className="mr-1" />}
